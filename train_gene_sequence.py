...
from utils.utils import load_pretraining_config
...
if __name__ == "__main__":
    ...
    config = load_pretraining_config("configs/pretrained_binary_bert_2432.json")
    ...
    os.environ["WANDB_PROJECT"] = "pretrained_binned_bert_2432_all_features"  # log to your project
    ...
    # tissue_label2id = tokenizer.tissue_type_label2id()

    # age_label2id = tokenizer.age_type_label2id()

    all_label2id = tokenizer.phenotypic_label2id()
    # cell_label2id = tokenizer.cell_type_label2id()
    ...
    #     roberta_config = RobertaConfig(vocab_size=len(tokenizer.vocab),
    #                                    max_position_embeddings=config.max_length)
    #
    #     model = RobertaForMaskedLM(config=roberta_config).to(device)
    # else:
    #     raise ValueError(f"model_name {config.model_name} is not supported")
