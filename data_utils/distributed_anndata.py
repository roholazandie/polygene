...
class DistributedAnnDataCollection(AnnCollection):
    ...
    def __init__(
        ...
        # schema
        adata0 = self.cache[self.filenames[0]] = read_h5ad_file(self.filenames[0], n_highly_variable_genes)
        # print(limits)
        assert len(adata0) == limits[0]
        self.schema = AnnDataSchema(adata0, obs_columns)
        # lazy anndatas
        lazy_adatas = [
            LazyAnnData(filename, (start, end), self.schema, self.cache, n_highly_variable_genes)
...
class LazyAnnData:
    ...
    def __init__(
        ...
        cache: LRU | None = None,
        highly_variable_genes: int | None = None,
    ):
        ...
        if cache is None:
            cache = LRU()
        self.cache = cache
        self.n_highly_variable_genes = highly_variable_genes
    ...
    def adata(self) -> AnnData:
        ...
        try:
            ...
            # fetch anndata
            adata = read_h5ad_file(self.filename, self.n_highly_variable_genes)
            # validate anndata
